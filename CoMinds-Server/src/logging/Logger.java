...
public class Logger {
	...
	private Logger(String fileName, LogLevel level, int queueSize) {
		this.level = level;
		this.worker = new LogWriter(fileName, queueSize);
	 ...
	 *            the size of the Buffer for the log messages
	 */
	private Logger(LogLevel level, int queueSize) {
		this.level = level;
		this.worker = new LogWriter(queueSize);
		new Thread(this.worker, "LoggerWriterThread").start();
	}
	...
	public static Logger getLogger(String fileName, LogLevel level,
			int queueSize) {
		if (singleton == null) {
			singleton = new Logger(fileName, level, queueSize);
		}
		return singleton;
	}

	/**
	 * gets a logger that logs not to a file, but to the console
	 * 
	 * @param level
	 *            the level of the logger
	 * @param queueSize
	 ...
	 * @return the logger
	 */
	public static Logger getLogger(LogLevel level, int queueSize) {
		if (singleton == null) {
			singleton = new Logger(level, queueSize);
		}
		return singleton;
	}
	...
	public static Logger getLogger() {
		if (singleton == null) {
			singleton = new Logger("log.txt", LogLevel.ERROR, 100);
		}
		return singleton;
	}
